...
describe("PieceStore", () => {
  ...
  describe("addPiece", () => {
    it("embeds content and stores in ChromaDB with encoded tags", async () => {
      ...
      const piece = await store.addPiece("Hello world", ["greeting", "test"]);
  ...
  describe("updatePiece", () => {
    it("updates content and re-embeds", async () => {
      ...
      const result = await store.updatePiece("id-1", "New content", ["new"]);
    ...
    it("updates only tags without re-embedding when content is undefined", async () => {
      ...
      const result = await store.updatePiece("id-1", undefined, ["newtag"]);
