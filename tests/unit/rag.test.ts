  ...
  describe("query", () => {
    it("retrieves pieces and generates an answer", async () => {
      mockQueryPieces.mockResolvedValueOnce([
        {
          piece: { id: "1", content: "TypeScript is typed JS", tags: ["ts"] },
          score: 0.9,
        },
      ]);
      mockChat.mockResolvedValueOnce({
        message: { content: "TypeScript is a typed superset of JavaScript." },
      });
    ...
    it("handles empty sources gracefully", async () => {
      mockQueryPieces.mockResolvedValueOnce([]);
      mockChat.mockResolvedValueOnce({
        message: { content: "I don't have enough context." },
      });

      const result = await rag.query("unknown topic");

      expect(result.sources).toEqual([]);
      expect(result.answer).toBe("I don't have enough context.");
    });
  });
});
